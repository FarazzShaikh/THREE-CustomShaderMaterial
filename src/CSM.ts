...
export class CSM extends Material {
  ...
  constructor(
    ...
    this.onBeforeCompile = (shader) => {
      if (parsedFragmentShdaer) {
        const patchedFragmentShdaer = this.patchShader(
          parsedFragmentShdaer,
      ...
      if (parsedVertexShdaer) {
        const patchedVertexShdaer = this.patchShader(
          parsedVertexShdaer,
          shader.vertexShader,
          PATCH_MAP.VERT
        );
  ...
  private parseShader(shader?: string): iCSMShader | undefined {
    ...
    const main = shader.match(
      /void\s*main\s*\w*\s*\([\w\s,]*\)\s*{([\w\W]*?)}/g
    );
