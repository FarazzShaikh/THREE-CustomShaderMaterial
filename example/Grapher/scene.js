...
import { pmremGenerator, renderer } from "./components.js";
...
function createBkgScene() {
  ...
  bkgScene.room.scale.setScalar(2);

  bkgScene.scene.add(bkgScene.room);

  bkgScene.mainLight = new THREE.PointLight(0xffffff, 0.15, 0, 2);

  bkgScene.scene.add(bkgScene.mainLight);

  //build the cube map fom this scene:

  var generatedCubeRenderTarget = pmremGenerator.fromScene(
    bkgScene.scene,
    0.02
  );
...
function buildScene(material) {
  ...
  const geometry = new THREE.PlaneGeometry(1, 1, 200, 400);
...
export { scene, buildScene, createEnvMap };
